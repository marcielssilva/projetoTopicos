...
def busca():
    ...
    return render_template("busca.html",
        ...
        categorias=categorias, resultados=resultados)
...
def _form_para_doc(form):
    ...
    # Slides do Canva (múltiplos)
    canva_titulos = form.getlist("canva_titulo")
    canva_links   = form.getlist("canva_link")
    slides_canva  = []
    for t, l in zip(canva_titulos, canva_links):
        if l.strip():
            slides_canva.append({
                "titulo": t.strip() or "Slides de Apoio",
                "link":   l.strip()
            })
